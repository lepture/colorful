...
var icons = {
  logIcon: colorful('➠ ').cyan.color,
  startIcon: colorful('⚑ ').bold.magenta.color,
  endIcon: colorful('➥ ').cyan.color
};
...
function log(context, level, args) {
  ...
  if (level === 'warn') {
    text += colorful(' WARN ').yellow_bg.color + ' ';
  } else if (level === 'error') {
    text += colorful(' ERROR ').red_bg.color + ' ';
    stream = process.stderr;
  }
  var colorize = function(text) {
    var name = colors[level];
    if (!name) return text;
    return colorful(text)[name].color;
  }
  if (colorize) {
...
Logging.prototype.start = function() {
    ...
    text += colorful(util.format.apply(this, arguments)).bold.color + os.EOL;
